...
)
{
    ...
    for (int way = 0; way < E; way++)
    {
        if ((*(cache + set) + way)->valid && (*(cache + set) + way)->tag == tag)
        {
            (*hit_count)++;
            hit = true;
            ((*(cache + set) + way)->recent_use) = program_counter;
            break;
        }
    }
    ...
    if (!hit)
    {
        ...
        for (int way = 0; way < E; way++)
        {
            if ((*(cache + set) + way)->recent_use < (*(cache + set) + remIndex)->recent_use)
                remIndex = way;
        }
        if (((*(cache + set) + remIndex)->valid))
            (*eviction_count)++;
        (*(cache + set) + remIndex)->tag = tag;
        (*(cache + set) + remIndex)->valid = true;
        ((*(cache + set) + remIndex)->recent_use) = program_counter;
    }
}
