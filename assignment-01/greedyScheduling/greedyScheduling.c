#include <stdlib.h>
#include <stdio.h>
#include <stdbool.h>

int main(int argc, char *argv[])
{
    FILE *fp = fopen(argv[1], "r");

    if (!fp)
    {
        perror("fopen failed");
        exit(EXIT_FAILURE);
    }

    char buf[256];

    // Read the number of jobs to be scheduled
    if (!fscanf(fp, "%s\n", buf))
    {
        perror("reading the number of jobs failed");
        exit(EXIT_FAILURE);
    }
    int jobcount = atoi(buf);

    // Next read the maximum number of timeslots
    if (!fscanf(fp, "%s\n", buf))
    {
        perror("reading the number of timeslots failed");
        exit(EXIT_FAILURE);
    }
    int timeslots = atoi(buf);

    // We've read the number of timeslots so close the file and start over
    fclose(fp);
    unsigned int prevEnd = 0;
    char prevChar;
    for (unsigned int slot = 0; slot < timeslots; slot++)
    {
        FILE *fp = fopen(argv[1], "r");
        if (!fp)
        {
            perror("fopen failed");
            exit(EXIT_FAILURE);
        }

        char buf[256];

        // First read the number of jobs to be scheduled
        if (!fscanf(fp, "%s\n", buf))
        {
            perror("reading the number of jobs failed");
            exit(EXIT_FAILURE);
        }

        // Next read the maximum number of timeslots
        if (!fscanf(fp, "%s\n", buf))
        {
            perror("reading the number of timeslots failed");
            exit(EXIT_FAILURE);
        }

        char wantedJob;
        unsigned int earliestEnd = timeslots + 1;
        unsigned int earliestStart = timeslots + 1;

        // Now read the rest of the file
        // read and save job charater from the while loop and determien if you want to run or not from there
        for (unsigned int line = 0; line < jobcount; line++)
        {
            if (!fgets(buf, 256, fp))
            {
                perror("reading a line for a job failed");
                exit(EXIT_FAILURE);
            }
            char job;
            unsigned int start;
            unsigned int end;
            if ((!sscanf(buf, "%c %d %d", &job, &start, &end)))
            {
                perror("parsing a line for a job failed");
                exit(EXIT_FAILURE);
            }
            if (start > prevEnd && end <= earliestEnd)
            {
                if (end == earliestEnd && earliestStart < start)
                {
                    continue;
                }
                else
                {
                    earliestEnd = end;
                    earliestStart = start;
                    wantedJob   = job;
                }
            }
        }
        prevEnd = earliestEnd;
        if (prevChar == wantedJob)
        {
            printf("\n");
            break;
        }
        prevChar = wantedJob;
        printf("%c\n", wantedJob);
        fclose(fp);
    }

    exit(EXIT_SUCCESS);
}
